...
hatch = toolpath_2d_hatch(
    boundary,                    # closed Polyline / Polygon -- outer boundary (stock top at z=0)
    spacing=tool.diameter * 0.6,  # float > 0 -- stepover between hatch lines
    angle=math.radians(30.0),    # float radians -- hatch line direction
    ...
    depth=12.0,                  # float -- layered roughing depth below z (0 = single flat pass)
    step=2.5,                    # None | float > 0 -- layer depth, descends helically per layer
)
print(hatch)
