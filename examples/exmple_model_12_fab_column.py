...
DRILL_LENGTH = 30.0  # mm -- override each hole axis to this length, extended at the top
...
end_cut = toolpath_ramp_path.from_outline(
...
toolpath = toolpath_2d_rectangle(line0, line1, RADIUS, safe_z=Z_SAFE)
...
for part in profile_clipped:
    ...
    ramp_profile = toolpath_ramp_path(mouth, Vector(0, 0, -beam_height), step=RAMP_STEP, safe_z=Z_SAFE)
...
for solid in cuts:
    ...
    drills.append(toolpath_helix_drill(axis, hole_radius, TOOL_DIAMETER, length=DRILL_LENGTH, floor=DRILL_FLOOR, safe_z=Z_SAFE))
...
for index, flip in zip(indices, flips):  # pick which cut solids to mill
    ...
    tp = toolpath_2d_rectangle.from_plate(cuts[index], RADIUS, safe_z=Z_SAFE, flip=flip, incline=True)
    if tp is None:
...
ramp = toolpath_ramp_line.from_box(cuts[9], step=RAMP_STEP, safe_z=Z_SAFE)
if ramp is None:
